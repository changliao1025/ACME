...
IceLndAtmOcn:
Y Test writing to json file, then running from it
Y test running solve from timing dir files
Y test for pulp
Y test can save json file if pulp is not available
Y Test running submit using X
Y test running solve from X
test with various options...

AtmLnd
test running old model from dictionary 
test running solve using X
"""
try:
...
class LoadBalanceTests(unittest.TestCase):
    ...
    def test_atm_lnd(self):
        ...
        with tempfile.NamedTemporaryFile('w+') as jsonfile1:
            ...
            jsonfile1.flush()
            cmd = "./load_balancing_solve.py --json_input %s --print_models" % (jsonfile1.name)
            output = run_cmd_no_fail(cmd, from_dir=CODE_DIR)
